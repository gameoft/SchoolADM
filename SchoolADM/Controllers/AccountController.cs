...
namespace SchoolADM.Controllers
{
    [Authorize]
                ...
                using (var identitydbContextTransaction = appDbContext.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        if (result.Succeeded)
                        {
                            ...
                            IdentityResult result3 = null;
                            if (0 == user.Claims.Where(c => c.ClaimType == ClaimTypes.DateOfBirth).Count())
                            {
                                result1 = await UserManager.AddClaimAsync(user.Id, new Claim(ClaimTypes.DateOfBirth, model.DOB));
                            }
                            if (0 == user.Claims.Where(c => c.ClaimType == ClaimTypes.StreetAddress).Count())
                            {
                                result2 = await UserManager.AddClaimAsync(user.Id, new Claim(ClaimTypes.StreetAddress, model.Address));
                            }
                            if (0 == user.Claims.Where(c => c.ClaimType == "MobilePhone").Count())
                            {
                                result3 = await UserManager.AddClaimAsync(user.Id, new Claim("MobilePhone", model.MobilePhone));
                            }
